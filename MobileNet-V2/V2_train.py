...
config = {
    ...
    'test_batch': 32,  # 测试集大小
    'net': V2_Net.MobileNetV2,  # 网络判断类别需在VGG16_train.py处更改
...
train_dataloader = DataLoader(V2_preprocess.train_data, config['train_batch'], shuffle=True, num_workers=4)
test_dataloader = DataLoader(V2_preprocess.test_data, config['test_batch'], shuffle=False, num_workers=4)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():  # 以下操作不影响网络参数
            ...
            if accuracy > best_acc:
                torch.save(net.state_dict(), 'weights/best_classify_v2.pt')

    torch.save(net.state_dict(), "weights/last_classify_v2.pt")
