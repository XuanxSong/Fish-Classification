...
import Squeeze10_preprocess
import Squeeze10_Net

# 超参数配置
config = {
    'train_batch': 32,  # 训练集大小
    'test_batch': 32,  # 测试集大小
    'net': Squeeze10_Net.SqueezeNet1_0,  # 网络判断类别需在VGG16_train.py处更改
...
train_dataloader = DataLoader(Squeeze10_preprocess.high_train, config['train_batch'], shuffle=True, num_workers=4)
test_dataloader = DataLoader(Squeeze10_preprocess.high_test, config['test_batch'], shuffle=False, num_workers=4)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():  # 以下操作不影响网络参数
            ...
            if accuracy > best_acc:
                torch.save(net.state_dict(), 'weights/best_classify_sn10_high.pt')

    torch.save(net.state_dict(), "weights/last_classify_sn10_high.pt")
