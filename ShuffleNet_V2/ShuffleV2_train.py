...
import Shuffle_x05_Net
import Shuffle_x10_Net
import Shuffle_x15_Net
import Shuffle_x20_Net
import ShuffleV2_preprocess
...
config = {
    ...
    'test_batch': 32,  # 测试集大小
    'net': Shuffle_x20_Net.ShuffleNet_x20,  # 网络判断类别需在VGG16_train.py处更改
...
train_dataloader = DataLoader(ShuffleV2_preprocess.high_train, config['train_batch'], shuffle=True, num_workers=4)
test_dataloader = DataLoader(ShuffleV2_preprocess.high_test, config['test_batch'], shuffle=False, num_workers=4)
...
def train_val(net, train, test, epochs, loss_fn, device):
    ...
    for epoch in range(epochs):
        ...
        with open("log/train_loss_x20_high.txt", 'a') as f:
        ...
        with torch.no_grad():  # 以下操作不影响网络参数
            ...
            if accuracy > best_acc:
                torch.save(net.state_dict(), 'weights/best_classify_x20_high.pt')

    torch.save(net.state_dict(), "weights/last_classify_x20_high.pt")
