...
train_dataloader = DataLoader(Res18_preprocess.high_train, config['train_batch'], shuffle=True, num_workers=4)
test_dataloader = DataLoader(Res18_preprocess.high_test, config['test_batch'], shuffle=False, num_workers=4)
...
def train_val(net, train, test, epochs, loss_fn, device):
    ...
    for epoch in range(epochs):
        ...
        for img, label, _ in tqdm(train):
            ...
            loss.backward()     # 此步骤计算梯度
            optimizer.step()    # 优化所有参数
        ...
        with open("log/train_loss_high_1.txt", 'a') as f:
        ...
        with torch.no_grad():  # 以下操作不影响网络参数
            ...
            if accuracy > best_acc:
                torch.save(net.state_dict(), 'weights/best_classify_high_a.pt')

    torch.save(net.state_dict(), "weights/last_classify_high_a.pt")
